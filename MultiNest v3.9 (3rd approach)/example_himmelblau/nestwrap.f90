...
subroutine nest_Sample(procsis, totalC, totalIter)

    implicit none

    integer nclusters				! total number of clusters found
    integer context
    integer maxNode, procsis, totalC, totalIter 				! variables used by the posterior routine

    ! calling MultiNest

    call nestRun(nest_IS,nest_mmodal,nest_ceff,nest_nlive,nest_tol,nest_efr,sdim,&
    nest_nPar,nest_nClsPar,nest_maxModes,nest_updInt,nest_Ztol,nest_root,&
    nest_rseed,nest_pWrap,nest_fb,nest_resume,nest_outfile,nest_initMPI,&
    nest_logZero,nest_maxIter,nest_minIter,getLogLike,dumper,context,&
    totalC,totalIter,procsis)
