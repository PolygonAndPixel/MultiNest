...
contains
  ...
  nest_updInt,nest_Ztol,nest_root,seed,nest_pWrap,nest_fb,nest_resume,nest_outfile,initMPI,nest_logZero,nest_maxIter,nest_minIter, &
  loglike,dumper,context, nest_totalLikelihoodCalls, nest_totalIterations, nest_procs)
        
  	implicit none
        
	integer nest_ndims,nest_nlive,nest_updInt,context,seed,i, nest_totalLikelihoodCalls, nest_totalIterations,nest_minIter,nest_procs
	...
	if(my_rank==0) then
		...
		if(ceff) write(*,'(a)')" running in constant efficiency mode"
      		...
      		write(*,*)"*****************************************************"
        write(*,*) 'Modified version. 1st approach.'
...
subroutine clusteredNest(p,phyP,l,loglike,dumper,context)
    ...
    if(my_rank==0) then
        ...
        allocate(pnewa(maxCls,procs,ndims),phyPnewa(maxCls,procs,totPar),lnewa(maxCls,procs), &
        sEll(procs),remain(maxCls),rIdx(maxCls))
...
!~~~~~~~~~~~~~~~~~~~~~~~~~END~OF~ADJUST~PRIOR~VOLUMES~~~~~~~~~~~~~~~~~~~~~~~~~~~                
                !now find a new point inside the hard constraint
                if(.not.eswitch) then
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~FIND~POINT~HARD~CONSTR~~~~~~~~~~~~~~~~~~~~~~~~~    
!-------------------------------parallel version--------------------------------
                    acpt = .false.
                    calls = 0
                    !$OMP PARALLEL num_threads(procs) SHARED(context, acpt) &
                    !$OMP& PRIVATE(id, n) REDUCTION(+:calls)
                    
                    do
                        id = OMP_get_thread_num() +1
                        call samp(pnewa(nd,id, :), &
                            phyPnewa(nd,id, :), &
                            lnewa(nd, id),sc_mean(1,:),d1, &
                            sc_tMat(1,:,:),ic_climits(nd,:,:),loglike,eswitch,lowlike,n,context, id)
                        calls = calls + n
                        !$OMP BARRIER
                        if(id == 1) then
                            ! In this approach we want to take the best point.
                            ! For each point evaluated by thread i, check if it has a higher
                            ! likelihood than the lowest and set the standard values accordingly.
                            do i=1, procs
                                numlike = numlike + 1
                                ! This point must have a higher likelihood than the current lowest 
                                ! one and there must be either no other point found with this 
                                ! restriction or this point has an higher likelihood.
                                if(lnewa(nd, i)>lowlike) then
                                    numlike = numlike - 1
                                    if(.not. acpt .or. lnewa(nd, i) > lnew) then
                                        pnew(:)=pnewa(nd, i, :)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~END~OF~FIND~POINT~HARD~CONSTR~~~~~~~~~~~~~~~~~~~~~~~~~
                else
!~~~~~~~~~~~~~~~~~~~~~~~FIND~POINT~HARD~CONSTR~ELLIPSOIDS~~~~~~~~~~~~~~~~~~~~~~~ 
!                     write(*,*) '----FIND~POINT~HARD~CONSTR~ELLIPSOIDS----'
!-------------------------------parallel version--------------------------------
                    num_old=numlike
                    calls = 0
                    !$OMP PARALLEL num_threads(procs) &
                    !$OMP& PRIVATE(id, d1, n) SHARED(acptBest) REDUCTION(+:calls)
                    id = OMP_get_thread_num() + 1
                    do
                        acpt=.false.
                        acptBest = .false.
                        ...
                        do
                            ...
                            call selectEll(ic_sc(nd),sc_vol(nd_i+1:nd_i+ic_sc(nd)),sEll(id), id-1)
                            sEll(id)=sEll(id)+nd_i
                            if(sc_kfac(sEll(id))==0.d0 .or. sc_vol(sEll(id))==0.d0) then
                                cycle
                            else
                                exit
                            endif
                        ...
                        d1=sc_kfac(sEll(id))*sc_eff(sEll(id))
                        call samp(pnewa(nd,id, :), phyPnewa(nd,id, :), &
                            lnewa(nd, id),sc_mean(sEll(id),:),d1, &
                            sc_tMat(sEll(id),:,:),ic_climits(nd,:,:),loglike,eswitch,lowlike,n,context,id)
                        calls = calls + n
                        !$OMP BARRIER
...
!                             write(*,*) '~ ~'
                            ...
                            do j1=1,OMP_get_num_threads()
                                numlike=numlike+1
                                if(lnewa(nd, j1)>lowlike .and. &
                                    (.not. acptBest .or. lnewa(nd, j1) > lnew)) then
                                    acpt = .false.
                                    if(sc_npt(sEll(j1))>0) then
                                        ...
                                        do m=nd_i+1,nd_i+ic_sc(nd)
                                            if(m==sEll(j1) .or. sc_npt(m)==0) cycle
                        ...
                        if(acptBest) then        
                            if(lnew>ic_hilike(nd) .and. id == 1) ic_hilike(nd)=lnew
                            if(id == 1) then
                                numlike = numlike + 1
                            endif
                            exit
                    ...
                    totalLikelihoodCalls = totalLikelihoodCalls + calls
!------------------------end of parallel version--------------------------------
!~~~~~~~~~~~~~~~~END~OF~FIND~POINT~HARD~CONSTR~ELLIPSOIDS~~~~~~~~~~~~~~~~~~~~~~~  
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~HANDLE~EV~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                    ...
                    if(mod(sff,updInt*10)==0 .or. ic_done(0)) call pos_samp(Ztol,globff,broot,nlive,ndims,nCdims,totPar,multimodal, &
...
!---------------------------------------------------------------------- 
  ...
  subroutine samp(pnew,phyPnew,lnew,mean,ekfac,TMat,limits,loglike,eswitch,lowlike,n,context, id)
	...
	do
			...
			call getrandom(ndims,pnew(1:ndims),idg)
			...
			call genPtInEll(ndims,mean,ekfac,TMat,idg,pnew(1:ndims))
  ...
  subroutine selectEll(n,volx,sEll, id)
