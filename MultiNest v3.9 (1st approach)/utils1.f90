...
!inv_cov=(evec).(inv_eval).Transpose(evec)
subroutine calc_invcovmatDebug(numdim,evec,eval,invcov)
    implicit none
    integer numdim !num of dimensions & points
    double precision evec(:,:),eval(:) !eigenvectors & eigenvalues
    double precision invcov(:,:)
...
end subroutine calc_invcovmatDebug
...
!---------------------------------------------------------------------- 
  ...
  !calculate the mean, covariance matrix, inv covariance matrix, evalues,
  !evects, det(cov) & enlargement of a given point set
subroutine CalcEllPropDebug(npt,ndim,pt,mean,covmat,invcov,tMat,evec,eval,detcov, &
    kfac,eff,vol,pVol,switch)
    implicit none
    !input variables
    integer npt !no. of points
...
!     if(sum(eval(:))/sum(eval(:)) /= sum(eval(:))/sum(eval(:))) then
!         write(*,*) 'eval is completely zero (or NaN):', eval(:)
!         write(*,*) 'evec', evec(:,:)
!         write(*,*) 'ndim', ndim
!         write(*,*) 'switch', switch
!         write(*,*) 'covmat', covmat(:,:)
...
!     write(*,*) 'Before calc_invcovmat: invcov(:,:)', invcov(:,:)
!     write(*,*) 'ndim', ndim
!     write(*,*) 'evec(:,:)', evec(:,:)
!     write(*,*) 'eval(:)', eval(:)
!     write(*,*) '~~~~~~~~~~~~~~~~~~~~~~~~~'
    ...
    !calculate the inverse of covariance matrix
    call calc_invcovmatDebug(ndim,evec,eval,invcov)
    
!     if(sum(invcov(:,:))/sum(invcov(:,:)) /= sum(invcov(:,:))/sum(invcov(:,:))&
!         .and. sum(invcov(:,:)) /= 0) then
!         write(*,*) 'After calc_invcovmat: invcov(:,:)', invcov(:,:)
!         write(*,*) 'ndim', ndim
!         write(*,*) 'evec(:,:)', evec(:,:)
!         write(*,*) 'eval(:)', eval(:)
!         stop
...
!     write(*,*) 'Before ScaleFactor: invcov(:,:)', invcov(:,:)
!     write(*,*) 'ndim', ndim
!     write(*,*) 'npt', npt
!     write(*,*) 'pt(:,:)', pt(:,:)
!     write(*,*) 'mean(:)', mean(:)
...
!     if(sum(invcov(:,:))/sum(invcov(:,:)) /= sum(invcov(:,:))/sum(invcov(:,:))&
!         .and. sum(invcov(:,:)) /= 0) then
!         write(*,*) 'After ScaleFactor: invcov(:,:)', invcov(:,:)
!         write(*,*) 'ndim', ndim
!         write(*,*) 'npt', npt
!         write(*,*) 'pt(:,:)', pt(:,:)
!         write(*,*) 'mean(:)', mean(:)
...
end subroutine CalcEllPropDebug
...
!generate a point uniformly inside the given ellipsoid
subroutine genPtInEllDEBUG(ndim,mean,efac,TMat,id,pt)
    implicit none
...
!     write(*,*) 'pt', pt
    ...
    call genPtInSpheroid(ndim,u(1,:),id)
!     write(*,*) 'Generated point from Spheroid:', u
    pnewM=MatMul(u,TMat)
    pt(:)=sqrt(efac)*pnewM(1,:)+mean(:)
...
!     write(*,*) 'mean', mean
!     write(*,*) 'pt', pt
end subroutine genPtInEllDEBUG
    ...
    if(a_r==1) then
        ...
        if(new_kfac==kfac) then
            ...
            if(kfac>new_kfac*eff .and. npt>1) then
                ...
                write(*,*) 'kfac from removed point: ', new_kfac
                write(*,*) 'New kfac from remaining points: ', kfac
                write(*,*) 'Original kfac: ', old_kfac
                write(*,*) 'kfac, new_kfac, eff, npt /', kfac,new_kfac,eff,npt
                write(*,*) 'mean', mean
                write(*,*) 'invcov', invcov
                write(*,*) 'ndim', ndim
                write(*,*) 'rejected point', newpt(:)
                write(*,*) 'pts'
                do i=1,npt 
                    write(*,*) pts(:,i)
                enddo
...
!----------------------------------------------------------------------
   ...
   !evolve an ellipsoid from which a point has either been rejected or a point
   !has been inserted
  subroutine evolveEllDebug(a_r,npt,ndim,newpt,pts,mean,eval,invcov,kfac,eff,vol,pVol, success)
